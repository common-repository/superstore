...
/**
 * Superstore posts rest api helper
 */
abstract class AbstractRestPostsController extends WP_REST_Controller {
	...
	protected function get_object( $id ) {
		...
		return new WP_Error( 'invalid-method', sprintf( __( "Method '%s' not implemented. Must be overridden in subclass.", 'superstore' ), __METHOD__ ), array( 'status' => 405 ) );
	}
	...
	public function get_item( $request ) {
		$validate = $this->validation_before_get_item( $request );
		...
		if ( ! empty( $post->post_type ) && 'product_variation' === $post->post_type && 'product' === $this->post_type ) {
			return new WP_Error( "superstore_rest_invalid_{$this->post_type}_id", __( 'To manipulate product variations you should use the /products/&lt;product_id&gt;/variations/&lt;id&gt; endpoint.', 'superstore' ), array( 'status' => 404 ) );
		} elseif ( empty( $id ) || empty( $post->ID ) || $post->post_type !== $this->post_type ) {
			return new WP_Error( "superstore_rest_invalid_{$this->post_type}_id", __( 'Invalid ID.', 'superstore' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		try {
			 ...
			 */
			do_action( "superstore_rest_insert_{$this->post_type}_object", $object, $request, true );
	...
	public function update_item( $request ) {
		...
		try {
			 ...
			 * @param boolean         $creating  True when creating object, false when updating.
			 */
			do_action( "superstore_rest_update_{$this->post_type}_object", $object, $request, false );

			/**
			 * Fires on post status change.
			 */
			do_action( "superstore_rest_update_{$this->post_type}_status_{$request['status']}", $object, $request['status'], false );
	...
	public function delete_item( $request ) {
		...
		do_action( "superstore_rest_delete_{$this->post_type}_object", $object, $response, $request );
	 ...
	 * @return boolean
	 */
	protected function validation_before_get_item( $request ) {
		return true;
	}
	...
	protected function prepare_objects_query( $request ) {
		...
		$args['author']       = ! isset( $request['seller_id'] ) ? get_current_user_id() : $request['seller_id'];
		...
		$args['posts_per_page']      = isset( $request['per_page'] ) ? $request['per_page'] : '-1';
		 ...
		 */
		$args = apply_filters( "superstore_rest_{$this->post_type}_object_query", $args, $request );
	 ...
	 *
	 * @param array           $prepared_args Args.
	 * @param WP_REST_Request $request Rest request.
	...
	protected function prepare_items_query( $prepared_args = array(), $request = null ) {
		...
		foreach ( $valid_vars as $var => $index ) {
			if ( isset( $prepared_args[ $var ] ) ) {
				$query_args[ $var ] = apply_filters( "superstore_rest_query_var_{$var}", $prepared_args[ $var ] );
			}
		}
	...
	protected function get_allowed_query_vars() {
		...
		$valid_vars = apply_filters( 'superstore_rest_query_vars', $valid_vars );
	 ...
	 *
	 * @param  SuperstoreData  $object  Object data.
	 * @param  WP_REST_Request $request Request object.
